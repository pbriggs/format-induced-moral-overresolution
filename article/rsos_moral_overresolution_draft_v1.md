...
Prior work has studied LLM moral judgment and norm prediction [1, 5], Moral Machine preferences across large language models [6, 7], SCRUPLES-style source-community norms and agreement [1, 4], calibrated confidence in model outputs [2, 3] and sensitivity to prompt formatting [8]. These studies ask how well model outputs match a reference label or distribution from one elicitation format. This study asks a distinct reliability question: when the same model answers the same case in two elicitation formats, do the resulting outputs communicate consistent levels of source-community agreement? Unlike standard calibration tests, this comparison is internal to the model's own outputs; it does not require the distributional estimate to have recovered the source-community distribution perfectly.

The failure mode this study tests is format-induced moral over-resolution: a verdict-style prompt can make a divided case appear more settled than the source-community votes (or the model's own distributional answer) would indicate. Three matched-item endpoints test whether and how much verdict-style outputs over-resolve source-community disagreement. Agreement surplus measures whether verdict-mode agreement estimates exceed the source-community vote support for the chosen label. The distribution-agreement gap is the central cross-format test: it measures whether those estimates also exceed the same model's own distribution-mode probability for the same label, comparing the same model, item and chosen label across elicitation formats. A positive gap is a cross-format inconsistency within the model's own outputs: the same model reports more apparent source-community agreement in verdict mode than its distributional answer assigns to that same label. Repeated forced-choice concentration measures whether repeated forced-choice outputs are more concentrated than the source-community vote distribution.

The study uses SCRUPLES Anecdotes, a corpus of everyday ethical situations with recorded votes on who was in the wrong [4]. "Source community" here refers to the annotator population represented in SCRUPLES; the resulting vote distributions are reference data from that data-generating process, not moral truth, universal norms or representative population estimates. In this preregistered audit, five large language models evaluated matched SCRUPLES items in distribution mode, verdict/agreement mode and repeated forced-choice mode (Fig. 1). Low-consensus items — cases with a majority label but substantial remaining disagreement in the source-community votes — were the primary confirmatory subset. Diffuse/no-clear-consensus items were analysed separately as secondary evidence; high-consensus items served as a reference condition. SCRUPLES provides measured disagreement in everyday moral judgments; the study's contribution is an evaluation of model-output reliability under format change.

The study contributes: (i) a preregistered five-model audit on a source-community disagreement dataset; (ii) matched distribution and verdict/agreement prompts applied to the same items and labels; (iii) the distribution-agreement gap, a within-model coherence test that holds model, item and label constant while the elicitation format changes; (iv) a repeated forced-choice concentration endpoint measuring label diversity across fresh forced-choice calls; and (v) release-safe reproducibility materials including derived analysis files, prompt templates and endpoint code.

This study does not claim the following. SCRUPLES votes are not moral truth and are not treated as such; they are a measured reference distribution from one annotator community. Source-community votes are not a global population estimate. The study does not measure user outcomes or behaviour in response to model verdicts. The study does not evaluate all large language models or all possible prompt formats. Training-data contamination was not directly tested, and no contamination claim is made.
...
Diffuse items showed a secondary gap of 0.226556 (95% CI, 0.216740-0.237164; n = 2,494). High-consensus items also showed a positive gap of 0.182842 (95% CI, 0.169762-0.196217; n = 1,745), with a low-consensus-minus-high-consensus contrast of 0.049845 (95% CI, 0.033774-0.065329). High-consensus items remained a positive reference condition; the larger low-consensus contrast indicates a greater coherence concern when source-community votes were more divided. Within the three-endpoint design, this gap provides the cleanest within-model coherence comparison: the mismatch is not between model outputs and source-community votes but between two outputs from the same model for the same case and label. Its positive value across all disagreement bins shows that cross-format coherence failure extends beyond divided cases; agreement surplus and repeated forced-choice concentration show that the magnitude of over-resolution is greatest where source-community votes are most divided.

### Repeated forced-choice outputs compress source-community disagreement

The first two endpoints concern stated agreement estimates. The third asks whether the same concentration pattern appears when models are asked only to choose labels repeatedly, without stating agreement. Repeated forced-choice concentration is source-community entropy minus the entropy of repeated forced-choice outputs on the five-label scale; positive values mean outputs were more concentrated across labels than source-community votes for the same items (see Methods for the collection protocol).
...
All five evaluated models showed positive low-consensus agreement surplus, positive low-consensus distribution-agreement gap and positive low-consensus repeated forced-choice concentration. Model-level low-consensus agreement surplus ranged from 0.326454 to 0.445325. Low-consensus distribution-agreement gaps ranged from 0.215187 to 0.271943. Low-consensus repeated forced-choice concentration ranged from 0.920001 to 1.408416. Leave-one-model-out summaries preserved the direction of the aggregate pattern for all three endpoints. These summaries support a consistent directional pattern across the five evaluated models; five models from the same deployment era cannot establish generalization across model families, training paradigms or deployment contexts. Provider, route and model-lineage comparisons remain outside the analysis.
...
As a dependence-aware robustness check, we recomputed the three low-consensus endpoints using model fixed effects plus item-cluster bootstrap uncertainty (4,000 iterations; seed `20260701`) on the frozen model-ready endpoint rows. The positive direction and similar magnitudes were preserved for all three endpoints: agreement surplus 0.370919 (95% CI, 0.360798-0.381290), distribution-agreement gap 0.232772 (95% CI, 0.224615-0.241494), and repeated forced-choice concentration 1.264638 bits (95% CI, 1.220329-1.307882). This analysis is a robustness check and does not replace the preregistered primary item-cluster bootstrap inference.
...
Operational milestones tracked monitoring and release discipline. The three primary endpoint definitions and the low-consensus confirmatory subset were specified in the preregistered protocol before any confirmatory data collection; they remained unchanged through the final 50k analysis. Subsequent presentation choices did not alter the frozen numerical outputs.
...
The source dataset was SCRUPLES Anecdotes, a corpus of everyday ethical situations with source-community judgments about who was in the wrong [4]. SCRUPLES Anecdotes was collected by Lourie, Le Bras and Choi at the Allen Institute for AI and released publicly with an associated GitHub repository and data-download instructions. Items were sourced from Reddit's r/AmITheAsshole community and annotated by crowd workers who judged who was in the wrong. The dataset was released under the terms described by the original authors; the source anecdotes should be accessed through the original dataset distribution subject to those terms. No new human participants were recruited for this study; no direct interaction with the original annotators or Reddit community members was undertaken. This study analysed previously collected, publicly released source-community vote counts and newly collected LLM outputs; it did not access any restricted, identifiable or special-category personal data beyond what is present in the released SCRUPLES distribution.

Secondary-data ethics determination: this is a computational audit using an existing publicly released dataset. No new participant recruitment, consent or data collection from human subjects was conducted. The study does not re-identify individuals, does not analyse special-category personal data for its own sake, and does not contact or interact with the source-community annotators or Reddit users. The original SCRUPLES data collection was reported with crowdworker annotation; the Allen Institute for AI's own ethics documentation and release terms govern use of the primary dataset. Raw SCRUPLES anecdotes and rendered prompts containing anecdote text are treated cautiously given potential privacy and sensitivity considerations and are not redistributed in unrestricted form.
...
For repeated sampling, the ten outputs for each item-model pair were ten separate fresh calls under the sampling prompt at temperature 0.0. Label order was assigned separately for each prompt assignment using the recorded seed procedure; `sample_id` was part of the local label-order seed. Invalid outputs were handled under the same parsing and validity rules as other components before entropy summaries were computed. The repeated forced-choice concentration endpoint measures diversity across repeated forced-choice outputs under the recorded prompt and label-order randomization protocol rather than stochastic sampling from an internal model distribution.
...
The four quantities involved in the three endpoints have distinct meanings and limitations:
...
Release-safe materials include frozen target lists and run manifests with sensitive details removed where needed; prompt templates and schemas; derived endpoint, robustness, paraphrase, baseline, secondary normative-certainty and distribution-quality summaries; bootstrap intervals, adjusted tests and validity/exclusion summaries; manuscript-table CSVs, figure-ready CSVs and rendered figures. The Supplementary Information and repository documentation list the main source-data and analysis files, and analysis code is described below. The repository licence is CC0 1.0 Universal unless superseded by OSF project settings.
...
Local setup and regeneration instructions are provided in `README.md`, `.env.example` and `docs/DATA_SETUP.md`. Offline regeneration uses completed-run data in an authorised local or reviewer environment and does not make new provider calls. Optional provider-call reruns require the rerunning researcher's own provider credentials and may produce new model outputs rather than exactly reproducing the frozen 50k collection.

## AI assistance disclosure

The author used AI tools to support editorial revision, code review, checklist preparation and manuscript organisation. The author reviewed, verified and takes responsibility for all analyses, claims, interpretations and final manuscript text. No AI tool is listed as an author.
