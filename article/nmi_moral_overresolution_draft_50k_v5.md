...
Large language models are increasingly asked to judge blame, responsibility and fairness in interpersonal situations. Some cases draw broad agreement; others do not. A reliable answer should not make a divided case sound settled. Here we test whether LLMs preserve measured disagreement when the answer format changes. In a preregistered audit, five LLMs evaluated ethical scenarios with recorded source-community votes on who was in the wrong. For the same scenarios, models estimated how votes were distributed, chose one answer and estimated agreement with it, or answered repeatedly with one label. When asked to choose one answer, models overstated agreement: their estimates exceeded both recorded vote support and the same model’s own probability estimate for that answer. Repeated one-label answers were also more concentrated than the votes, with the clearest concern in low-consensus cases. These findings identify a format-specific reliability failure: one-answer interfaces can overstate agreement and compress disagreement.
...
We call the tested failure mode **format-induced moral over-resolution**: a format-specific reliability failure in which a divided source-community judgment distribution becomes a more resolved agreement signal when the model is asked for a verdict. We test that shift with three matched-item endpoints. They ask whether the format shift inflates apparent agreement relative to source-community support, makes reported agreement exceed the same model's distribution-mode probability for the selected label, or compresses repeated forced-choice outputs. The distribution-agreement gap is the most direct cross-format coherence test because it holds the model, item and selected label fixed while only the elicitation format changes. These endpoints test uncertainty transfer; they do not require perfect distribution calibration.

We use SCRUPLES Anecdotes because it provides everyday ethical situations with source-community judgments about who was in the wrong [4]. Here, "source community" refers to the annotator/judgment population represented in SCRUPLES, and the resulting distributions are source-community reference distributions from that data-generating process rather than moral truth, universal norms, representative population estimates or deployment-ready normative standards. In this preregistered audit, five LLMs evaluated matched SCRUPLES items in distribution mode, descriptive verdict/agreement mode and repeated-sampling mode (Fig. 1). Low-consensus items were the primary confirmatory subset; diffuse/no-clear-consensus items were analysed separately as secondary evidence; high-consensus items served as a reference condition.

The contribution is threefold. First, the study operationalizes cross-format uncertainty transfer for source-community moral judgments. Second, it defines matched-item endpoints for surplus agreement relative to the source-community distribution, within-model incoherence across output formats, and compression in repeated forced-choice outputs, with the distribution-agreement gap isolating the same-model, same-item, same-label comparison most directly. Third, it shows in a preregistered five-model audit that verdict-style elicitation inflates apparent agreement and compresses disagreement, with the clearest reliability concern when source-community judgments are divided.
...
Table 1 and Methods report the model roster, component allocation and scale of the study, including 47,500 target calls and 47,432 primary-valid outputs.
...
The second endpoint is the most direct uncertainty-transfer test within the model's own outputs. For each matched comparison, the model, item and verdict-selected label were fixed; only the elicitation format changed. A positive distribution-agreement gap means the verdict/agreement format reported higher estimated source-community agreement for the selected label than the same model assigned to that label in distribution mode.

This comparison tests cross-format output coherence: whether two answers from the same model communicate compatible levels of source-community agreement for the same item-label pairing. Because the comparison is internal to the model's own outputs, it is not reducible to imperfect calibration against the SCRUPLES source-community distribution. Perfect calibration of the distribution-mode estimate is not required.
...
Diffuse items showed a secondary gap of 0.226556 (95% CI, 0.216740-0.237164; n = 2,494). High-consensus items also showed a positive gap of 0.182842 (95% CI, 0.169762-0.196217; n = 1,745). The low-consensus-minus-high-consensus contrast was 0.049845 (95% CI, 0.033774-0.065329). High-consensus items therefore remained a positive reference condition, while the low-consensus contrast indicates a larger coherence problem when source-community agreement was weaker. Within the three-endpoint design, this same-model, same-item, same-label gap anchors the uncertainty-transfer claim: the apparent agreement signal changes when the output format changes.
...
This endpoint extends the evidence beyond stated agreement. Under the study's repeated forced-choice protocol, outputs across fresh calls were more concentrated than source-community judgments, with larger compression in the low-consensus primary subset than in the high-consensus reference condition. It measures diversity across repeated fresh calls under the recorded prompt and label-order randomization protocol rather than stochastic sampling from an internal model distribution.
...
Robustness checks evaluated data handling, reference-distribution construction, item composition, distribution-mode degeneracy and prompt surface form. They support the three endpoint results above while keeping the primary endpoints as the three low-consensus tests.

A strict-valid-only analysis addressed parsing and schema heterogeneity by restricting the data to outputs that satisfied the strict schema. The direction of all three low-consensus endpoints was preserved: agreement surplus 0.376030, distribution-agreement gap 0.223867 and sampling compression 1.268188.

The source-community reference distribution was also checked against alternative constructions from SCRUPLES vote counts. Low-consensus endpoint effects were recomputed using raw proportions, Jeffreys smoothing and Laplace smoothing, and positive low-consensus effects were preserved across these constructions. This sensitivity analysis concerns reference-distribution construction rather than calibration of distribution-mode model outputs to those distributions.
...
This study identifies a representation-to-interface failure in LLM uncertainty communication. In the evaluated models under the tested prompt formats, uncertainty elicited in distribution mode did not reliably survive a verdict-style interface. The clearest evidence is the distribution-agreement gap: for the same model, same item and same verdict-selected label, verdict/agreement mode reported stronger apparent source-community agreement than the model's own distribution-mode probability assigned to that label. This within-model comparison shows how a model can appear uncertainty-aware in one format while communicating a sharper social-consensus signal in another.

The result concerns uncertainty transfer and output reliability over source-community reference distributions. When the reference distribution is divided, a verdict-style prompt can make the output appear more settled than either the source-community distribution or the model's own distribution-mode estimate supports.

This distinction matters for evaluation and benchmark design. Majority-label accuracy can reward a model for selecting the most common label while missing whether it communicates the disagreement around that label. Distribution-mode uncertainty tests are also incomplete on their own. Passing such a test is insufficient if the same model's verdict/agreement output communicates stronger source-community agreement for the same item and label. Evaluations of LLM reliability should therefore be format-sensitive: prior work shows that benchmark conclusions can depend on prompt formatting [8], and the present results extend that concern to whether reported uncertainty remains coherent when the interface changes from a distributional answer to a verdict, summary or estimate of agreement.

The findings also matter for interface design. Users often ask for verdicts in settings where source-community disagreement is relevant to the task. In such settings, distributional interfaces, explicit disagreement summaries and checks on verdict/agreement outputs may reduce the risk that a divided source-community reference distribution is turned into an overconfident agreement signal. The design constraint is direct: verdict-style interfaces should not make divided source-community judgments sound more settled than the reference distribution or the model's own distributional answer supports for the same item and label.
...
The paraphrase audit addresses aggregate surface-form sensitivity rather than training-set membership; contamination was not directly measured, and the small matched original-vs-paraphrase overlap limits paired interpretation. Item familiarity could affect absolute label probabilities; the central within-item comparison asks whether the same model's verdict-mode agreement estimate exceeds its independently elicited distribution-mode probability for the same item and label. As a model-output reliability audit, the study leaves user effects on perceived social consensus, confidence and decision-making as an empirical question.

The implication for model-output reliability is concrete: uncertainty should be evaluated in the formats where users encounter model outputs. A credible uncertainty-aware evaluation should ask both whether a model can produce non-degenerate distributions on request and whether its verdict/agreement interface preserves compatible source-community agreement signals for the same item and label. Otherwise, evaluation can certify uncertainty in the distributional format that asks for it while missing over-resolution in the verdict-style format users may actually see.
