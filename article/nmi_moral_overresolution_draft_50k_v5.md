...
The paraphrase audit addresses aggregate surface-form sensitivity rather than training-set membership; contamination was not directly measured, and the small matched original-vs-paraphrase overlap limits paired interpretation. Although item familiarity could affect absolute label probabilities, it would not by itself explain the within-item, same-model pattern in which a model's verdict-mode agreement estimate exceeds its independently elicited distribution-mode probability for the same item and label. As a model-output reliability audit, the study leaves user effects on perceived social consensus, confidence and decision-making as an empirical question.
