...
We use SCRUPLES Anecdotes because it provides everyday ethical situations with source-community judgments about who was in the wrong [4]. Here, "source community" refers to the annotator/judgment population represented in SCRUPLES. The resulting distributions are source-community reference distributions from that data-generating process, not moral truth, universal norms, representative population estimates or deployment-ready normative standards. In this preregistered audit, five LLMs evaluated matched SCRUPLES items in distribution mode, descriptive verdict/agreement mode and repeated-sampling mode (Fig. 1). Low-consensus items were the primary confirmatory subset; diffuse/no-clear-consensus items were analysed separately as secondary evidence; high-consensus items served as a reference condition.
...
Within the limited matched original-vs-paraphrase overlap, chosen-label stability was 0.655 in low-consensus rows and 0.467 in diffuse rows. The matched coverage (29 low-consensus rows and 15 diffuse rows) makes these results aggregate surface-form evidence rather than a definitive paired original-vs-paraphrase test. Contamination was not directly measured.
...
The findings are bounded by one source-community dataset, five evaluated models and a finite set of prompt templates. The source community is not a universal or representative population sample.

Contamination was not directly measured. The paraphrase audit addresses surface-form sensitivity rather than training-set membership, and the small matched original-vs-paraphrase overlap makes the paraphrase results aggregate surface-form evidence only. Although item familiarity could affect absolute label probabilities, it would not by itself explain the within-item, same-model pattern in which a model's verdict-mode agreement estimate exceeds its independently elicited distribution-mode probability for the same item and label. As a model-output reliability audit, the study leaves user effects on perceived social consensus, confidence and decision-making as an empirical question.

The implication for model-output reliability is concrete: uncertainty should be evaluated in the formats where users encounter model outputs. A credible uncertainty-aware evaluation must ask whether a model's verdict/agreement interface preserves compatible source-community agreement signals for the same item and label, not only whether it can produce non-degenerate distributions on request. Otherwise, evaluation can certify uncertainty in the distributional format that asks for it while missing over-resolution in the verdict-style format users may actually see.
