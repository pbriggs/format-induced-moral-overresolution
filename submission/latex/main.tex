...
\hypersetup{pdftitle={Verdict-style prompts induce moral over-resolution in large language models},pdfauthor={Paul Briggs}}
\Urlmuskip=0mu plus 2mu\relax
\sloppy
\title{Verdict-style prompts induce moral over-resolution in large language models}
...
\subsubsection{Endpoint logic and target set}\label{endpoint-logic-and-target-set}
...
\subsubsection{Verdict prompts inflate apparent agreement}\label{verdict-prompts-inflate-apparent-agreement}
...
\subsubsection{Verdict agreement exceeds the model's distributional estimate}\label{verdict-agreement-exceeds-the-models-distributional-estimate}
...
\subsubsection{Repeated forced-choice outputs compress source-community disagreement}\label{repeated-forced-choice-outputs-compress-source-community-disagreement}
...
\subsubsection{Pattern across models and output validity}\label{pattern-across-models-and-output-validity}
...
Item-composition checks asked whether the pattern depended on annotation density or on the \texttt{info} label. The directional endpoint pattern was preserved in high-annotation-only analyses, after excluding \texttt{info}-majority items and after excluding high-\texttt{info} items.
...
\subsubsection{Preregistration and analysis scope}\label{preregistration-and-analysis-scope}
...
\subsubsection{Dataset, eligibility and label schema}\label{dataset-eligibility-and-label-schema}
...
The \texttt{info} label was retained in the primary distributions because removing it would alter the observed disagreement structure.

\subsubsection{Source-community distributions, bins and item allocation}\label{source-community-distributions-bins-and-item-allocation}
...
Item selection and label-order assignment used the recorded milestone seed \texttt{20260615}. Within each disagreement bin, eligible items were selected according to the frozen allocation plan and component-specific target counts; the released target manifests record the resulting item IDs. Beyond the eligibility rules above, target-list membership was determined by the frozen allocation plan and recorded seed rather than by inspection of model outputs.
...
\subsubsection{Model roster and collection}\label{model-roster-and-collection}

The amended frozen roster contained five models: \texttt{claude-sonnet-4-6}, \texttt{deepseek/deepseek-v3.2}, \texttt{gpt-5.5}, \texttt{grok-4.3} and \texttt{qwen/qwen3.7-max}. Each model had 9,500 target calls. API routes and call windows are reported in Table 1a.
...
\subsubsection{Prompt modes and request construction}\label{prompt-modes-and-request-construction}
...
\subsubsection{Parsing, validity and exclusions}\label{parsing-validity-and-exclusions}
...
Primary-valid outputs were 47,432 of 47,500: 38,790 strict-schema outputs and 8,642 extracted-JSON outputs. Invalid status counts were 52 \texttt{probability\_out\_of\_bounds} outputs, 10 \texttt{probability\_sum\_error} outputs, 4 invalid-JSON outputs and 2 empty responses. Refusals and off-schema labels were zero. Repair fields were tracked, but \texttt{repair\_attempted=0} and \texttt{repair\_successful=0}; the final 50k analysis contained no repaired outputs. Validity by model and prompt mode is reported in Extended Data Table 1.
...
\subsubsection{Endpoint definitions}\label{endpoint-definitions}
...
Primary inference used item-cluster bootstrap confidence intervals with 2,000 bootstrap iterations and seed \texttt{20260621}. For bin-specific estimates, each bootstrap replicate resampled item IDs with replacement within the bin and retained all relevant model/prompt rows associated with the resampled items.
...
\subsubsection{Robustness and secondary analyses}\label{robustness-and-secondary-analyses}

Robustness analyses covered five areas: parsing and schema validity, source-community distribution construction, annotation and \texttt{info} sensitivity, distribution-mode non-degeneracy, and prompt surface form. Checks included strict-valid-only subsets; raw, Jeffreys-smoothed and Laplace-smoothed source-community distributions; high-annotation-only subsets; exclusion of \texttt{info}-majority and high-\texttt{info} items; distribution-quality diagnostics with non-API baselines; and aggregate paraphrase analyses.
...
\subsection{Ethics and human-subjects statement}\label{ethics-and-human-subjects-statement}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{claude-sonnet-4-6} & Anthropic & \url{https://api.anthropic.com/v1/messages} & 2026-06-16T22:25:08.996871+00:00 & 2026-06-21T15:33:15.162406+00:00 & 9,500 \\
\texttt{deepseek/deepseek-v3.2} & OpenRouter-compatible route & \url{https://openrouter.ai/api/v1/chat/completions} & 2026-06-16T22:25:04.892615+00:00 & 2026-06-21T15:33:02.923140+00:00 & 9,500 \\
\texttt{gpt-5.5} & OpenAI & \url{https://api.openai.com/v1/responses} & 2026-06-16T22:25:12.406257+00:00 & 2026-06-21T15:33:16.432159+00:00 & 9,500 \\
\texttt{grok-4.3} & xAI & \url{https://api.x.ai/v1/responses} & 2026-06-17T17:55:15.846572+00:00 & 2026-06-21T15:32:59.334062+00:00 & 9,500 \\
\texttt{qwen/qwen3.7-max} & OpenRouter-compatible route & \url{https://openrouter.ai/api/v1/chat/completions} & 2026-06-16T22:25:15.735026+00:00 & 2026-06-21T15:33:13.991444+00:00 & 9,500 \\
\end{longtable}
...
\textbf{Table 1b \textbar{} Target allocation by study component.}
...
\textbf{Table 2 \textbar{} Primary endpoint estimates, contrasts, confidence intervals and adjusted tests.}
...
Endpoint
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Low-consensus primary mean (95\% CI)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Holm-adjusted P
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Diffuse secondary mean (95\% CI)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
High-consensus reference mean (95\% CI)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Low--high contrast (95\% CI)
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\subsection{Extended Data and Supplementary Information}\label{extended-data-and-supplementary-information}
...
\textbf{Extended Data Fig. 1 \textbar{} Distribution-quality diagnostics.} Diagnostic distances and entropy summaries by source-community disagreement bin and model show that distribution-mode outputs varied by item and bin and were not merely uniform or fixed base-rate responses. This is a diagnostic support analysis, not a distribution-prediction benchmark.

\textbf{Extended Data Fig. 2 \textbar{} Aggregate paraphrase-audit effects.} Aggregate paraphrase-audit agreement surplus and distribution-agreement gap estimates preserve the direction of the main effects. The figure should be interpreted with the limited matched-coverage caveat stated in the Results and Supplementary Information.

\textbf{Extended Data Fig. 3 \textbar{} Validity by model and prompt mode.} Primary-valid output rates by model and prompt mode show high validity and limited invalid outputs in the frozen 50k target-scoped analysis.

\textbf{Extended Data Table 1 \textbar{} Validity and exclusion summary.} Reports validity by model/mode, invalid status counts, minimum model/mode validity, refusal count, off-schema count, API errors, terminal failures and repair fields.

\textbf{Supplementary Table 1 \textbar{} Full model roster, API parameters, model-level endpoints and leave-one-model-out summaries.} Provides model provenance, API-parameter reporting and cross-model consistency checks without making provider-family or model-family claims.

\textbf{Supplementary Table 2 \textbar{} Source-community distribution smoothing robustness.} Reports raw-proportion, Jeffreys-smoothed and Laplace-smoothed source-community distribution sensitivity checks.

\textbf{Supplementary Table 3 \textbar{} Annotation and \texttt{info} robustness.} Reports high-annotation-only, \texttt{info}-majority exclusion and high-\texttt{info} exclusion checks.

\textbf{Supplementary Table 4 \textbar{} Paraphrase audit.} Reports paraphrase target calls, valid outputs, aggregate effects, confidence intervals, chosen-label stability and the matched-coverage caveat.

\textbf{Supplementary Table 5 \textbar{} Distribution quality and baselines.} Reports distribution-quality diagnostics and uniform, global-base-rate and source-majority-oracle baselines.

\textbf{Supplementary Table 6 \textbar{} Normative certainty.} Reports the secondary moral-certainty construct; moral certainty is not equivalent to estimated source-community agreement.

\textbf{Supplementary Table 7 \textbar{} Validity and exclusions.} Reports validity and exclusion summaries and cross-references Extended Data Table 1.

\textbf{Supplementary Table 8 \textbar{} Model-ready secondary rows.} Provides model-ready secondary rows as transparency files; these files do not contain reported mixed-effects model results.
