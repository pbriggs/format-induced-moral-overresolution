...
\title{Supplementary Information: Verdict-style prompts over-resolve disagreement in large language model moral judgments}
\author{Paul Briggs\\Independent researcher, Los Angeles, CA, USA}
\date{}
\begin{document}
\maketitle
\linenumbers
...
\section{Supplementary Note 1: Prompt templates, response schemas and hashes}
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.18\linewidth}>{\raggedright\arraybackslash}p{0.18\linewidth}}
 \toprule
Component & SHA-256 \\
 \midrule
\path{src/prompts/prompt_templates.py} & \path{7f673dd3ca14d3780b53c05d16b67df2af98c7c750fc97256938a9976f32599b} \\
\path{src/prompts/schemas.py} & \path{58a3c91e42cb8264253f1c705a6c6d2162405fc6167e61c9ec28f5eefef02619} \\
\path{src/prompts/prompt_hashing.py} & \path{9153ce8e1a34cfd2fb897343eaecb4b7e327460b3c4c101535bc2306b80ee491} \\
\path{src/protocol/label_schema.py} & \path{d743eee461e80b7b4cafcbaebf8e1a1b0db15bcc98f29f7d467824b01a581369} \\
\path{src/protocol/prompt_modes.py} & \path{f6a2f157a1d120677a385db4aea022621e0dd816700c57f853561469d3735bce} \\
\path{src/utils/hashing.py} & \path{772528eb9234091835c9b9cd6f38dbc22384649c754b05645d248e78426cc707} \\
 \bottomrule
\end{longtable}
\normalsize
...
The canonical label schema is \path{scruples_five_label_v1}, with five labels: \path{author}, \path{other}, \path{everybody}, \path{nobody} and \path{info}. The prompt modes are distribution mode, descriptive verdict/agreement mode, normative verdict mode, repeated-sampling mode, paraphrase generation, paraphrased distribution mode and paraphrased descriptive verdict mode. Recognition-audit prompt and schema definitions are present in the code release as unused definitions; they were not executed, analysed or used to support any claim, and no formal recognition audit was run for the final 50k analysis.

Distribution-mode outputs require \path{label_probabilities} and \path{most_likely_label}. Descriptive verdict/agreement outputs require \path{chosen_label} and \path{estimated_source_community_agreement}. Normative-verdict outputs require \path{chosen_label} and \path{moral_certainty}. Repeated-sampling-mode outputs require \path{chosen_label}. Schemas use fixed label enumerations and disallow additional top-level properties where applicable.

Stable schema hashes are: distribution mode, \path{62225e53095e01dded2b558b031b5d246c3dfcf403d660a0f12ef6c5d074f9ff}; descriptive verdict/agreement mode, \path{75cefb0c22603127c15e101628358e4e64a7462ee662080e37667ca439077284}; normative verdict mode, \path{b61f82be27531ac964f7291b0fc2e73a3e045f7561965f35d84da0205324554b}; repeated-sampling mode, \path{6a31e2aad4d6a2d3a852b213d99ad09c7c2bbbf716e8e3ee9402d61da7c0ed68}; recognition audit, \path{c0cbb3f41df3ee3f2e4339ecd0ecbe2224cba33069df3935b47ac889e4bce44d}; paraphrase generation, \path{29c4483a9f7a7677861cb301f35536d35ac9f88bc9e69ae30e2ca945d970b826}; paraphrased distribution mode, \path{62225e53095e01dded2b558b031b5d246c3dfcf403d660a0f12ef6c5d074f9ff}; and paraphrased descriptive verdict/agreement mode, \path{75cefb0c22603127c15e101628358e4e64a7462ee662080e37667ca439077284}. The recognition-audit hash is listed only because the schema definition is part of the code release; it does not indicate collection or analysis of recognition-audit outputs.
...
Item subset selection and label-order assignment used seed \path{20260615}. For each prompt assignment, the local label-order seed was derived as the first 16 hex characters of the stable hash of \path{[seed, item_id, model_id, mode, sample_id]}, converted to an integer and passed to Python's \path{random.Random}; the resulting label order and assignment hash were stored with each request. The post-50k manifest reports 120 unique label orders in the paraphrase audit.

---

\section{Supplementary Note 2: Dataset provenance, label schema and source-community distributions}
...
The five canonical labels were retained in the primary analysis, including \path{info} for not enough information/cannot tell. Retaining \path{info} preserves the observed five-label disagreement structure rather than forcing uncertain cases into one of the four substantive blame labels.
...
\section{Supplementary Note 3: Sampling, disagreement bins and target allocation}
...
\section{Supplementary Note 4: Model roster, API parameters and collection windows}

The amended frozen roster contains five models: \path{claude-sonnet-4-6}, \path{deepseek/deepseek-v3.2}, \path{gpt-5.5}, \path{grok-4.3} and \path{qwen/qwen3.7-max}. Each model has 9,500 target calls. API routes, first and last call timestamps, temperature and top-p fields are reported in \path{post_run/analysis_exports/50k/manuscript_tables/table_model_roster_50k.csv}.
...
\section{Supplementary Note 5: Parsing, validity, repair fields and exclusions}
...
\section{Supplementary Note 6: Endpoint definitions, quantity caveats and bootstrap inference}
...
\section{Supplementary Note 9: Distribution-quality diagnostics and baselines}
...
\section{Supplementary Note 10: Normative-certainty secondary analysis}
...
\section{Supplementary Note 11: Analysis provenance}
...
Supplementary Table & What it supports & Source data \\
 \midrule
Supplementary Table 1 & Full model roster, API parameters, model-level endpoints and leave-one-model-out summaries supporting model provenance and cross-model consistency checks. & \path{post_run/analysis_exports/50k/manuscript_tables/table_model_roster_50k.csv`; `post_run/analysis_exports/50k/model_level_endpoint_table_50k.csv`; `post_run/analysis_exports/50k/leave_one_model_out_50k.csv} \\
...
Supplementary Table 3 & Annotation-count and \path{info} robustness checks supporting the treatment of annotation density and the retained \path{info} label. & \path{post_run/analysis_exports/50k/manuscript_tables/table_robustness_annotation_info_50k.csv} \\
Supplementary Table 4 & Paraphrase-audit aggregate effects, confidence intervals, chosen-label stability and matched-coverage caveat. & \path{post_run/analysis_exports/50k/manuscript_tables/table_paraphrase_effects_50k.csv`; `post_run/analysis_exports/50k/manuscript_tables/table_paraphrase_ci_50k.csv} \\
Supplementary Table 5 & Distribution-quality diagnostics and baselines supporting non-degeneracy and item sensitivity of distribution-mode outputs. & \path{post_run/analysis_exports/50k/manuscript_tables/table_distribution_quality_50k.csv`; `post_run/analysis_exports/50k/manuscript_tables/table_baseline_distribution_quality_50k.csv} \\
...
Supplementary Table 7 & Validity by model/mode, invalid-output categories, minimum validity rates, refusals, off-schema labels, API errors, terminal failures and repair fields. & \path{post_run/analysis_exports/50k/manuscript_tables/table_validity_by_model_mode_50k.csv`; `post_run/analysis_exports/50k/manuscript_tables/table_invalid_output_summary_50k.csv} \\
Supplementary Table 8 & Model-ready endpoint rows and dependence-aware robustness summary (model fixed effects plus item-cluster bootstrap) used as a sensitivity check, not as primary inference. & \path{post_run/analysis_exports/50k/model_ready/mixed_effects_endpoint_rows_50k.csv`; `post_run/analysis_exports/50k/model_ready/mixed_effects_endpoint_rows_strict_valid_50k.csv`; `post_run/analysis_exports/50k/model_fixed_effects_item_cluster_bootstrap_50k.csv} \\
 \bottomrule
\end{longtable}
\normalsize
\setlength{\tabcolsep}{6pt}
...
\section{Extended Data figures and source data}
...
Extended Data Fig. 2 & Aggregate paraphrase-audit effects for agreement surplus and distribution-agreement gap, interpreted with the limited matched-coverage caveat. & \path{post_run/analysis_exports/50k/rendered_figures/figure_paraphrase_audit_effects_50k.svg`; `post_run/analysis_exports/50k/figure_ready/figure_paraphrase_effects_by_bin_model_50k.csv} \\
Extended Data Fig. 3 & Primary-valid output rates by model and prompt mode, supporting the validity and exclusion summary. & \path{post_run/analysis_exports/50k/rendered_figures/figure_validity_rate_by_model_50k.svg`; `post_run/analysis_exports/50k/figure_ready/figure_validity_by_model_mode_50k.csv} \\
Extended Data Table 1 & Validity by model/mode and invalid-output summary, including repair fields and exclusion categories. & \path{post_run/analysis_exports/50k/manuscript_tables/table_validity_by_model_mode_50k.csv`; `post_run/analysis_exports/50k/manuscript_tables/table_invalid_output_summary_50k.csv} \\
 \bottomrule
\end{longtable}
\normalsize
\setlength{\tabcolsep}{6pt}
...
\section{Public and restricted materials}
